...
async function main() {
  ...
  if (!affDevis.ok) {
    ...
    var devisWeek = dDevis.semaines.find(function (s) { return s.chantiers.some(function(c) { return c.includes("_TEST_DEVIS"); }); });
    assert(devisWeek === undefined, "DEVIS_ENVOYE affaire must NOT appear in chantiers");
    assert(!dDevis.semaines.some(function(s) { return s.type !== "libre" && s.chantiers.some(function(c) { return c.includes("_TEST_DEVIS"); }); }), "DEVIS_ENVOYE must not contribute to sold weeks");
  ...
  if (!affAcc.ok) {
    ...
    var accWeek = dAcc.semaines.find(function (s) { return s.chantiers.some(function(c) { return c.includes("_TEST_ACCEPTEE"); }); });
