...
pool.connect().then(async (client) => {
  try {
    ...
    // app.devis_accept_token_sha256 matches the row's CONDENSAT column.
    // This enables the unauthenticated acceptance flow without bypassing RLS.
    //
    // La colonne `accept_token` en clair a été SUPPRIMÉE par la migration 014 :
    // ce script recréait une policy qui la référençait, et échouait donc sur
    // toute base à jour. Il compare désormais le condensat, comme la migration.
