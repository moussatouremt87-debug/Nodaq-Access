...
ownerPool.connect().then(async (client) => {
  try {
    ...
    //
    // ── CONNECT d'abord, et il n'est PAS facultatif ─────────────────────────
    // Sur un PostgreSQL auto-hébergé, le rôle PUBLIC détient CONNECT par
    // défaut : app_user en hérite, et l'oubli ne se voit jamais.
    //
    // Les hébergeurs managés le RÉVOQUENT. Sur Scaleway, l'application démarre,
    // atteint la base, et échoue à la vérification de rôle avec :
    //
    //   permission denied for database "…"  (42501)
    //   User does not have CONNECT privilege.
    //
    // Le message n'accuse ni le mot de passe ni le réseau, et le rôle existe
    // bel et bien — d'où le temps perdu à chercher ailleurs. Rencontré au
    // premier déploiement Scaleway, le 28/08/2026.
    //
    // Le nom de la base vient de l'URL, jamais d'une constante : Scaleway
    // nomme la sienne `rdb`, un conteneur local `nodaq`, la CI `nodaq_ci`.
    const nomBase = new URL(process.env.DATABASE_URL).pathname.replace(/^\//, "");
    const grants = [
      `GRANT CONNECT ON DATABASE "${nomBase}" TO app_user`,
    ...
    // journal_decisions (US-A6.4) : la preuve de ce que l'humain a décidé sur
    // les propositions de l'assistant. Un journal de contrôle que l'application
    // pourrait réécrire ne prouverait rien.
    // usage_franchissements (migration 065) : un franchissement de seuil
    // constaté ne se réécrit pas — c'est l'unicité (tenant, mois, seuil) qui
    // garantit qu'une alerte ne part qu'une fois.
    const APPEND_ONLY_TABLES = ["archived_pdfs", "envois_journal", "objectifs_franchissements", "paiements", "contact_bases", "oppositions", "incidents_facturation", "pa_documents_recus", "journal_decisions", "regles_relance", "usage_franchissements", "essai_jalons"];
    ...
    // 3quater. Référentiels en lecture seule pour l'application. La grille
    // tarifaire (migration 065) interdit de coder ou de modifier un prix hors
    // du seed de migration — comme pour archived_pdfs, c'est le moteur qui
    // porte la règle, pas le code applicatif.
    const READ_ONLY_TABLES = ["plans"];
    for (const table of READ_ONLY_TABLES) {
      ...
      if (rows[0]?.present) {
        await client.query(`REVOKE INSERT, UPDATE, DELETE ON ${table} FROM app_user`);
        console.log(`✓ ${table}: lecture seule (INSERT/UPDATE/DELETE revoked)`);
      }
    }
