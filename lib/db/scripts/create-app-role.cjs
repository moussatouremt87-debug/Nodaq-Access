...
ownerPool.connect().then(async (client) => {
  try {
    ...
    if (!exists) {
      await client.query(`CREATE ROLE app_user LOGIN PASSWORD '${safePw}'`);
      console.log("✓ app_user created");
    } else {
      await client.query(`ALTER ROLE app_user WITH LOGIN PASSWORD '${safePw}'`);
      console.log("✓ app_user password reset");
    }
