...
if (!connectionString) {
  console.error("[migrate] ERROR: DATABASE_URL is required (owner/superuser credentials).");
  console.error("  export DATABASE_URL=postgres://owner:password@host:5432/dbname");
  process.exit(1);
}
...
if (caPem) {
  if (!caPem.includes("BEGIN CERTIFICATE")) {
    console.error("[migrate] ERROR: DATABASE_CA_PEM ne ressemble pas à un certificat PEM.");
    process.exit(1);
  }
  let hote = "";
  try { hote = new URL(connectionString).hostname; } catch { /* laissé vide */ }
...
async function listMigrationFiles() {
  ...
  try {
    ...
    if (err.code === "ENOENT") {
      ...
      console.error(`[migrate] FATAL: Migrations directory not found: ${MIGRATIONS_DIR}`);
      if (process.env.MIGRATIONS_DIR) {
        console.error(`  MIGRATIONS_DIR env var is set to: ${process.env.MIGRATIONS_DIR}`);
        console.error(`  Ensure the directory exists inside the container/environment.`);
      } else {
        console.error(`  Set MIGRATIONS_DIR env var to the directory containing *.sql files.`);
  ...
  if (files.length === 0) {
    console.error(`[migrate] FATAL: Migrations directory is empty: ${MIGRATIONS_DIR}`);
...
async function run() {
  ...
  try {
    ...
    console.log(`[migrate] ${pending.length} migration(s) to apply: ${pending.join(", ")}`);
