/**
 * Le brief — « ce qui s'est passé depuis hier, ce qui attend aujourd'hui ».
 *
 * ── POURQUOI CE CALCUL A QUITTÉ LA ROUTE ────────────────────────────────────
 *
 * Il vivait entièrement dans `routes/brief.ts`. Tant que l'écran était le seul
 * consommateur, cela allait. Le brief part maintenant aussi par courriel, sans
 * que personne ouvre l'application — et deux façons de décider « ce qui compte
 * aujourd'hui » finiraient par se contredire : l'artisan lirait une urgence
 * dans sa boîte, et ne la retrouverait pas à l'écran.
 *
 * C'est le défaut qui a coûté le plus cher dans ce dépôt, sous toutes ses
 * formes : trois définitions du chiffre d'affaires, six de « l'affaire
 * active », deux du « en retard ». À chaque fois, une copie a divergé et l'une
 * des deux mentait.
 *
 * La logique est donc déplacée SANS ÊTRE MODIFIÉE. La route l'appelle et rend
 * son résultat ; l'envoi du matin appelle la même fonction.
 */
...
export interface Brief {
  ...
  readonly sections: readonly SectionBrief[];
}

export async function composerBrief(tenantId: string): Promise<Brief> {
  ...
  return { date: todayStr, greeting, sections };
}
