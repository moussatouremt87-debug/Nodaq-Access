...
router.get("/prospects", async (req, res): Promise<void> => {
  ...
  const prospects = await withTenant(tenantId, async (tx) => {
    let query = tx.select().from(prospectsTable).$dynamic();
    if (stage) query = query.where(eq(prospectsTable.stage, stage));
    return query.orderBy(desc(prospectsTable.createdAt));
  });
