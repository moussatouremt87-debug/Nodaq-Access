...
import { db, affairesTable, facturesTable, prospectsTable } from "@workspace/db";
...
router.get("/rapports/mensuel", async (req, res): Promise<void> => {
  ...
  const allAffaires = await db.select().from(affairesTable);
  const allFactures = await db.select().from(facturesTable);
  const allProspects = await db.select().from(prospectsTable);

  // Affaires created THIS month
  const moisAffaires = allAffaires.filter(a => {
    const d = new Date(a.createdAt);
    return d >= monthStart && d < monthEnd;
  });

  // Factures issued THIS month
  const facturesMois = allFactures.filter(f => {
    const d = new Date(f.issuedDate);
    return d >= monthStart && d < monthEnd;
  });
  ...
  // New clients = prospects reaching GAGNE stage THIS month
  const nouveauxClients = allProspects.filter(p => {
    const d = new Date(p.updatedAt ?? p.createdAt);
    return d >= monthStart && d < monthEnd && p.stage === "GAGNE";
  }).length;

  // Taux de marge: based on affaires with margin data created THIS month
  ...
  // Top affaires THIS month by invoiced amount
  const topAffaires = moisAffaires
    ...
    .map(a => ({
      ...
      marginPct: a.invoicedAmountCents && a.invoicedAmountCents > 0
        ? Math.round(((a.marginCents ?? 0) / a.invoicedAmountCents) * 1000) / 10
        : null,
    }));

  // Top clients by total facturé THIS month
  const clientMap = new Map<string, number>();
  facturesMois.forEach(f => {
    clientMap.set(f.customerName, (clientMap.get(f.customerName) ?? 0) + f.amountCents);
  });
