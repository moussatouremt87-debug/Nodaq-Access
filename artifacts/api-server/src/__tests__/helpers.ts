...
export function tableInsertSql(table: string, tenantId: string, memberAId?: string): [string, unknown[]] {
  ...
  const map: Record<string, [string, unknown[]]> = {
    ...
    // cr_entries has composite PK (line_code, period_key, tenant_id) — no id column
    cr_entries:         [`INSERT INTO cr_entries (line_code, period_key, tenant_id) VALUES ('RLS_TEST', '2020-01-01:2020-12-31', $1) ON CONFLICT DO NOTHING`, [tenantId]],
    ...
    absences:           memberAId
      ? [`INSERT INTO absences (membre_id, date_debut, date_fin, tenant_id) VALUES ($1, $2, $2, $3) ON CONFLICT DO NOTHING`, [memberAId, now, tenantId]]
