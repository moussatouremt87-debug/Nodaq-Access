...
describe("US-A4.1 — rattachement d'un créneau de la semaine type", () => {
  test("un créneau avec clientId seul est créé et relu tel quel", async () => {
    ...
    const { body: liste } = await request(serveurTest(app)).get("/api/equipe").set("Cookie", cookie).expect(200);
...
describe("US-A4.3 — typeLien : coûté, jamais compté dans la capacité", () => {
  ...
  test("l'angle mort corrigé : un sous-traitant disponible n'augmente pas la capacité affichée sur /equipe/plannings", async () => {
    const avant = await request(serveurTest(app)).get("/api/equipe/plannings").set("Cookie", cookie).expect(200);
    ...
    const apresSousTraitant = await request(serveurTest(app)).get("/api/equipe/plannings").set("Cookie", cookie).expect(200);
    ...
    const apresSalarie = await request(serveurTest(app)).get("/api/equipe/plannings").set("Cookie", cookie).expect(200);
...
describe("US-A4.4 — habilitations d'un membre", () => {
  test("création, lecture (statut calculé) et suppression", async () => {
    const membre = await request(serveurTest(app))
      .post("/api/equipe")
      .set("Cookie", cookie)
      .send({ name: "Membre Habilitation" })
      .expect(201);

    const creee = await request(serveurTest(app))
    ...
    const suppr = await request(serveurTest(app))
      .delete(`/api/equipe/${membre.body.id}/habilitations/${creee.body.id}`)
      .set("Cookie", cookie);
    expect(suppr.status).toBe(204);

    const { body: listeApres } = await request(serveurTest(app)).get("/api/equipe").set("Cookie", cookie).expect(200);
