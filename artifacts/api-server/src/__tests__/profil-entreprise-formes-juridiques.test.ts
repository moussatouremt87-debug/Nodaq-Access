...
async function patchProfil(l: Locataire, updates: Record<string, string>): Promise<void> {
  await request(app).patch("/api/onboarding/profil").set("Cookie", l.cookie).send(updates).expect(200);
}
...
describe("franchise en base de TVA (art. 293 B CGI)", () => {
  ...
  test("mention automatique, pas de ligne Total TVA", async () => {
    ...
    await request(app).post(`/api/factures/${f.id}/emettre`).set("Cookie", l.cookie).send({}).expect(200);
    const pdfRes = await request(app).get(`/api/factures/${f.id}/pdf`).set("Cookie", l.cookie).expect(200);
  ...
  test("profil franchise + ligne à taux non nul → émission bloquée", async () => {
    ...
    const res = await request(app).post(`/api/factures/${f.id}/emettre`).set("Cookie", l.cookie).send({}).expect(422);
  ...
  test("s'applique aussi à un avoir (régression duplication loadSellerInfo)", async () => {
    const f = await creerFacture(l, [{ vatRate: 0, vatCategory: "E" }]);
    await request(app).post(`/api/factures/${f.id}/emettre`).set("Cookie", l.cookie).send({}).expect(200);
    const { body: avoir } = await request(app).post("/api/avoirs").set("Cookie", l.cookie)
      .send({ factureRefId: f.id, montantHtCents: 50_000, montantTvaCents: 0, motif: "Remise commerciale" })
      .expect(201);
    const pdfRes = await request(app).get(`/api/avoirs/${avoir.id}/pdf`).set("Cookie", l.cookie).expect(200);
...
describe("capital social et RCS", () => {
  ...
  test("s'applique aussi à un devis (régression duplication chargerEmetteur)", async () => {
    const { body: devis } = await request(app).post("/api/devis").set("Cookie", l.cookie)
      .send({ clientName: "Client Devis", lines: [{ description: "Étude", quantity: 1, unitPriceCents: 50_000 }] })
      .expect(201);
    const pdfRes = await request(app).get(`/api/devis/${devis.id}/pdf`).set("Cookie", l.cookie).expect(200);
  ...
  test("absents → aucune mention capital/RCS", async () => {
    ...
    await request(app).post(`/api/factures/${f.id}/emettre`).set("Cookie", l2.cookie).send({}).expect(200);
    const pdfRes = await request(app).get(`/api/factures/${f.id}/pdf`).set("Cookie", l2.cookie).expect(200);
...
describe("GET /onboarding/profil renvoie les nouvelles clés", () => {
  test("une fois posées via PATCH", async () => {
    ...
    const { body } = await request(app).get("/api/onboarding/profil").set("Cookie", l.cookie).expect(200);
